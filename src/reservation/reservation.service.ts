...
export class ReservationService {
  ...
  async getAllReservation(req: Request) {
    ...
    const processedReservations = await Promise.all(
      reservations.map(async (element) => {
        ...
        // format booking time
        const formattedBookingTime = format(
        ...
        if (element.status === 0) element['Active'] = 1;
        else if (element.status === 1) element['Pending'] = 1;
        else if (element.status === 2) element['End'] = 1;
        else if (element.status === 3) element['Completed'] = 1;
        return element;
      }),
    );
  ...
  async create(reqData: any, res: Response) {
    try {
      ...
      if (room.available === 0) {
        res
          .cookie('error', 'This room is no longer available')
